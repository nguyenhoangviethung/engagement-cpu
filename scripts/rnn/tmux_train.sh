...
conda run --no-capture-output -n "$CONDA_ENV" env PYTHONPATH="$WORKDIR/src" python -m engagement_daisee.rnn.train$sample_flag --log-every "$LOG_EVERY" --output "$run_checkpoint" --run-id "$active_run_id" --model "$MODEL_NAME" --cpu-threads "$CPU_THREADS" 2>&1 | tee -a "$run_log"
