...
usage() {
  cat <<'EOF'
Usage: scripts/rnn/tmux_eval.sh [command] [options]
...
mkdir -p "$LOG_DIR"
LATEST_LOG_LINK="$LOG_DIR/latest_eval.log"

build_command() {
  local timestamp="$(date +%Y%m%d_%H%M%S)"
  local run_log="$LOG_DIR/eval_${timestamp}.log"
...
echo "=== Eval finished at \$(date) ===" | tee -a "$run_log"
ln -sfn "$run_log" "$LATEST_LOG_LINK"
EOF
}
