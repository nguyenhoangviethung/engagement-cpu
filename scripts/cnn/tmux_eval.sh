...
SESSION_NAME="engagement_cnn_eval"
CONDA_ENV="thesis"
...
usage() {
  cat <<'EOF'
Usage: scripts/cnn/tmux_eval.sh [command] [options]
...
mkdir -p "$LOG_DIR"
LATEST_LOG_LINK="$LOG_DIR/latest_cnn_eval.log"

build_command() {
  local timestamp="$(date +%Y%m%d_%H%M%S)"
  local run_log="$LOG_DIR/cnn_eval_${timestamp}.log"
...
echo "=== CNN eval example started at \$(date) ===" | tee -a "$run_log"
...
echo "=== CNN eval example finished at \$(date) ===" | tee -a "$run_log"
...
echo "=== CNN eval started at \$(date) ===" | tee -a "$run_log"
conda run --no-capture-output -n "$CONDA_ENV" env PYTHONPATH="$WORKDIR/src" python -m engagement_daisee.cnn.evaluate \\
  --manifest "$MANIFEST" \\
  --checkpoint "$CHECKPOINT" \\
  --split "$SPLIT" \\
  --batch-size "$BATCH_SIZE"$threshold_flag$output_flag 2>&1 | tee -a "$run_log"
echo "=== CNN eval finished at \$(date) ===" | tee -a "$run_log"
ln -sfn "$run_log" "$LATEST_LOG_LINK"
EOF
}
